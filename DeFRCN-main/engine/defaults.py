...
from defrcn.evaluation import DatasetEvaluator, inference_on_dataset,  inference_on_dataset_nopcb, print_csv_format, verify_results, inference_on_dataset_n
...
def default_argument_parser():
    ...
    parser.add_argument('--threshold', type=float, required = False, default = 0.9999)
    parser.add_argument("--objectness",
                    action='store_true',
                    default=False,
                    help="Enable objectness. Default is False")
    parser.add_argument("--groundtruth",
                    action='store_true',
                    default=False,
                    help="Enable groundtruth. Default is False")
...
def default_setup(cfg, args):
    ...
    with pm.open(args.config_file, "r") as file:
            file_content = file.read()
    logger.info("Command line arguments: " + str(args))
    if hasattr(args, "config_file"):
        logger.info(
            "Contents of args.config_file={}:\n{}".format(
                args.config_file, file_content
                    ,
            )
        )
...
class DefaultPredictor:
    ...
    def __init__(self, cfg):
        ...
        print("model weights: {}".format(cfg.MODEL.WEIGHTS))
...
class DefaultTrainer(SimpleTrainer):
    ...
    def __init__(self, cfg):
        ...
        self.cfg = cfg
        self.max_iter = 800 if self.max_iter is None else self.max_iter
    ...
    def resume_or_load(self, resume=True):
        ...
        self.start_iter = 0 if self.start_iter is None else self.start_iter
        self.start_iter = (
    ...
    def test(cls, cfg, model, evaluators=None):
        ...
        for idx, dataset_name in enumerate(cfg.DATASETS.TEST):
            evaluator2 = cls.build_evaluator(cfg, dataset_name)
            ...
            results_i, results_j = inference_on_dataset_nopcb(model, data_loader, evaluator, evaluator2, cfg)
            results[dataset_name] = results_i
            results2[dataset_name] = results_j
            if comm.is_main_process():
                assert isinstance(
                    results_j, dict
                ...
                print("pcb yokken: \n")
                print_csv_format(results_j)
                print("pcb varken: \n")
                print_csv_format(results_i)
        if len(results2) == 1:
            results2 = list(results2.values())[0]
        return results
...
class Trainer(SimpleTrainer):
    ...
    def test(cls, cfg, model, evaluators=None):
        ...
        for idx, dataset_name in enumerate(cfg.DATASETS.TEST):
            ...
            last_change = inference_on_dataset_n(model, data_loader, evaluator, cfg)
        return last_change
...
class Features:
    ...
    def __call__(self, original_image):
        ...
        self.resnet_features = FeatureExtractor(self.model, layers = ["backbone.res2.0.conv1","backbone.res4.22.conv3","affine_rcnn","affine_rpn"])
        inputs = {"image": image, "height": height, "width": width}
        predictions = self.resnet_features([inputs])
        return predictions