...
def main(args):
    ...
    f_pcb.close()
    if args.groundtruth:
        filename = f"groundtruth_{args.interval}_{args.video}_objectness{args.objectness}.txt"
    else:
        filename = f"{args.threshold}_{args.interval}_{args.video}_objectness{args.objectness}.txt"
    cfg = setup(args)
    args.resume = False
    last_change = 0
    while last_change != -1:
        ...
        for idx, dataset_name in enumerate(cfg.DATASETS.TEST):
            ...
            # last_change, best frame'in frame number(idx)'i
    ...
    with open(filename, 'w') as file:
      for item in change:
          file.write("%s\n" % item)
    os.rename("results_pcb_tu.txt", f"results_{args.video}_{args.groundtruth}_{args.threshold}_{args.interval}_objectness{args.objectness}.txt")
